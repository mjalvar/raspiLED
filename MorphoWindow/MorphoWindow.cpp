...
MorphoWindow::MorphoWindow() :
    ...
    // Add drawing area
    m_VBox.pack_start(gtk,Gtk::PACK_SHRINK);
    ...
    m_ButtonStart.signal_clicked().connect(sigc::mem_fun(*this, &MorphoWindow::on_start_button_clicked));
    m_ButtonStop.signal_clicked().connect(sigc::mem_fun(*this, &MorphoWindow::on_stop_button_clicked));
    m_ButtonQuit.signal_clicked().connect(sigc::mem_fun(*this, &MorphoWindow::on_quit_button_clicked));

    // Connect the handler to the dispatcher.
    m_Dispatcher.connect(sigc::mem_fun(*this, &MorphoWindow::on_notification_from_worker_thread));
...
void MorphoWindow::on_start_button_clicked() {
    if (m_WorkerThread) {
        ...
        m_WorkerThread = new std::thread(
            [this] {
                m_Worker->do_work(this,&gtk);
            });
    }
    update_start_stop_buttons();
}

void MorphoWindow::on_stop_button_clicked() {
    if (!m_WorkerThread) {
...
void MorphoWindow::update_start_stop_buttons() {
...
void MorphoWindow::update_widgets() {
...
void MorphoWindow::on_quit_button_clicked() {
    if (m_WorkerThread) {
...
// notify() is called from MorphoWork::do_work(). It is executed in the worker
// thread. It triggers a call to on_notification_from_worker_thread(), which is
// executed in the GUI thread.
void MorphoWindow::notify() {
    m_Dispatcher.emit();
}

void MorphoWindow::on_notification_from_worker_thread() {
